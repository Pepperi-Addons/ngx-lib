import {
    PepTextboxComponent,
    PepTextboxModule,
} from '@pepperi-addons/ngx-lib/textbox';
...
import {
    PepAttachmentComponent,
    PepAttachmentModule,
} from '@pepperi-addons/ngx-lib/attachment';
import {
    PepCheckboxComponent,
    PepCheckboxModule,
} from '@pepperi-addons/ngx-lib/checkbox';
import {
    PepColorComponent,
    PepColorModule,
} from '@pepperi-addons/ngx-lib/color';
import { PepDateComponent, PepDateModule } from '@pepperi-addons/ngx-lib/date';
import { PepGroupButtonsModule } from '@pepperi-addons/ngx-lib/group-buttons';
import {
    PepImageComponent,
    PepImageModule,
} from '@pepperi-addons/ngx-lib/image';
import {
...
} from '@pepperi-addons/ngx-lib/quantity-selector';
import {
...
import {
    PepSelectComponent,
    PepSelectModule,
} from '@pepperi-addons/ngx-lib/select';
import {
    PepSeparatorComponent,
    PepSeparatorModule,
} from '@pepperi-addons/ngx-lib/separator';
import {
    PepSignatureComponent,
    PepSignatureModule,
} from '@pepperi-addons/ngx-lib/signature';
import {
    PepTextareaComponent,
    PepTextareaModule,
} from '@pepperi-addons/ngx-lib/textarea';
...
export class AppModule {
    constructor(translate: TranslateService, private injector: Injector) {
        let userLang = 'en';
    ...
    ngDoBootstrap() {
        const textboxElement = createCustomElement(PepTextboxComponent, {
            injector: this.injector,
        });
        customElements.define('pep-textbox', textboxElement);
        const attachmentElement = createCustomElement(PepAttachmentComponent, {
            injector: this.injector,
        });
        customElements.define('pep-attachment', attachmentElement);
        const checkboxElement = createCustomElement(PepCheckboxComponent, {
            injector: this.injector,
        });
        customElements.define('pep-checkbox', checkboxElement);
        const colorElement = createCustomElement(PepColorComponent, {
            injector: this.injector,
        });
        customElements.define('pep-color', colorElement);
        const dateElement = createCustomElement(PepDateComponent, {
            injector: this.injector,
        });
        customElements.define('pep-date', dateElement);
        const imageElement = createCustomElement(PepImageComponent, {
            injector: this.injector,
        });
        customElements.define('pep-image', imageElement);
        const imagesFilmstripElement = createCustomElement(
            PepImagesFilmstripComponent,
            { injector: this.injector }
        );
        ...
        const menuElement = createCustomElement(PepMenuComponent, {
            injector: this.injector,
        });
        customElements.define('pep-menu', menuElement);
        const quantitySelectorElement = createCustomElement(
            PepQuantitySelectorComponent,
            { injector: this.injector }
        );
        ...
        const selectElement = createCustomElement(PepSelectComponent, {
            injector: this.injector,
        });
        customElements.define('pep-select', selectElement);
        const separatorElement = createCustomElement(PepSeparatorComponent, {
            injector: this.injector,
        });
        customElements.define('pep-separator', separatorElement);
        const signatureElement = createCustomElement(PepSignatureComponent, {
            injector: this.injector,
        });
        customElements.define('pep-signature', signatureElement);
        const textareaElement = createCustomElement(PepTextareaComponent, {
            injector: this.injector,
        });
