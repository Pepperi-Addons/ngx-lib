...
export class PepListChooserComponent implements OnInit {
    ...
    set options(value: Array<IPepListChooserOption>) {
        ...
        if (this.menuItems?.length === 1) {
            this.breadCrumbs = [];
            this.breadCrumbs.push(new PepBreadCrumbItem(this.menuItems[0].text));
        }
    }
    ...
    @Output() change: EventEmitter<IPepListChooserOptionChangeEvent> = new EventEmitter<IPepListChooserOptionChangeEvent>();
    ...
    onMenuItemClicked(menuItemClickEvent: IPepMenuItemClickEvent): void {
        const currentList = this.options.find(list => list.key === menuItemClickEvent.source.key);
